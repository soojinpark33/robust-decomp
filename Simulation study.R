...
#true value of dic for Tables 1
tau_dic <- coef(lm(Y ~ R + X + C , data=popdat))[2]
unexp_dic <- coef(lm(Y ~ R + X + C + M , data=popdat))[2]
#true value of kob for Tables 1
tau_ox <- coef(lm(Y~R , data=popdat))[2]  
exp_ox <- delta[3]*coef(lm(M~R , data=popdat))[2]
unexp_ox <- tau_ox-exp_ox 
# true value of cda for Tables 1
...
#true value of DIC for TAble 2; cannot obtain with popdat1; used popdat2
#true value of kob for Table 2 
tau_ox <- coef(lm(Y~R , data=popdat1))[2]  
exp_ox <- delta[3]*coef(lm(M~R , data=popdat1))[2]
unexp_ox <- tau_ox-exp_ox 
# true value of cda for Table 2
...
#true value of DIC for TAble 2
tau_dic <- coef(lm(Y~R +X + C, data=popdat2))[2]  
unexp_dic <- coef(lm(Y~R +X + M+ C +U, data=popdat2))[2] 
exp_dic <- tau_dic- unexp_dic 
#true value of kob for Table 3
tau_ox <- coef(lm(Y~R , data=popdat2))[2]  
exp_ox <- delta[3]*coef(lm(M~R , data=popdat2))[2]
unexp_ox <- tau_ox-exp_ox 
# true value of cda for Table 3
tau_cda <- coef(lm(Y~R +C, data=popdat2))[2]   
exp_cda <- delta[3]*coef(lm(M~R + C, data=popdat2))[2] 
unexp_cda <- tau_cda-exp_cda 
#_________________
...
#::::::::::::::::::::::::::::::::
## Adjusted CDA
# partial correlation Corr(M, U | R,X,C)
...
# partial correlation Corr(Y, U | R,X,M,C)
